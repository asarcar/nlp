...
def attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    def attention(query):
      ...
      as_list = [] # Debug Attention Softmax output
    ...
    if initial_state_attention:
      attns, attns_smax = attention(initial_state)
      attns_smax_list += attns_smax
    for i, inp in enumerate(decoder_inputs):
      if i > 0:
        variable_scope.get_variable_scope().reuse_variables()
      ...
      if i == 0 and initial_state_attention:
        with variable_scope.variable_scope(
          ...
          attns, attns_smax = attention(state)
          attns_smax_list += attns_smax
      else:
        attns, attns_smax = attention(state)
        attns_smax_list += attns_smax
  ...
  return outputs, state, attns_smax_list
  ...
  with ops.name_scope(name, "model_with_buckets", all_inputs):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(
        ...
        bucket_outputs, _, bucket_attns = seq2seq(
        ...
        if per_example_loss:
          ...
          losses.append(
              sequence_loss(
                  ...
                  weights[:bucket[1]],
                  softmax_loss_function=softmax_loss_function))
        attns.append(bucket_attns)

  assert len(attns) == len(buckets)
  assert len(attns[-1]) == buckets[-1][1]
