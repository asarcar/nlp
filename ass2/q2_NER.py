...
class Config(object):
  ...
  label_size = 5
  hidden_size = 100
  max_epochs = 25 # set 25 when running production
  early_stopping = 2
  dropout = 0.9
  lr = 0.001
  l2 = 0.001
...
class NERModel(LanguageModel):
  ...
  def load_data(self, debug=False):
    ...
    if debug:
      self.X_train = self.X_train[:num_debug]
      self.y_train = self.y_train[:num_debug]
    ...
    self.X_dev, self.y_dev = du.docs_to_windows(
        docs, word_to_num, tag_to_num, wsize=self.config.window_size)
    if debug:
      self.X_dev = self.X_dev[:num_debug]
      self.y_dev = self.y_dev[:num_debug]
    ...
    self.X_test, self.y_test = du.docs_to_windows(
        docs, word_to_num, tag_to_num, wsize=self.config.window_size)
    if debug:
      self.X_test = self.X_test[:num_debug]
      self.y_test = self.y_test[:num_debug]
  ...
  def add_placeholders(self):
    ...
    self.input_placeholder  = tf.placeholder(
      ...
      name="input")
    # Label: PER, ORG, LOC, MISC, and O (Other):
    # enumerated type interpreted as float32 to support compatible tensor operations
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      rnd_uni_initializer = tf.random_uniform_initializer(
        minval=-1.0, maxval=1.0)
      with tf.variable_scope("Embed", initializer=rnd_uni_initializer) as scope: 
        vocab_size = len(self.wv)
        embed_shape= [vocab_size, self.config.embed_size]
        embeddings = tf.get_variable("embeddings", embed_shape) 
      win = tf.nn.embedding_lookup(embeddings, self.input_placeholder)
      embed_width = self.config.window_size*self.config.embed_size
      window = tf.reshape(win, [-1, embed_width], name="embed_window")
      ### END YOUR CODE
      return window
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer", initializer=xavier_weight_init()) as scope: 
      wt_shape = [self.config.window_size*self.config.embed_size, self.config.hidden_size]
      W        = tf.get_variable("W", wt_shape)
      b1_shape = [self.config.hidden_size]
      b1       = tf.get_variable("b1", b1_shape)
      layer    = tf.add(tf.matmul(window, W, name="matmul-layer"), b1, name="add-layer")
      h_pre    = tf.tanh(layer, name="tanh-layer")
    h        = tf.nn.dropout(h_pre, self.dropout_placeholder)
    ...
    with tf.variable_scope("Softmax", initializer=xavier_weight_init()) as scope: 
      ...
      U        = tf.get_variable("U", u_shape)
      b2_shape = [self.config.label_size]
      b2       = tf.get_variable("b2", b2_shape)
      op       = tf.add(tf.matmul(h, U, name="matmul-softmax"), b2, name="add-softmax")
    output   = tf.nn.dropout(op, self.dropout_placeholder)

    if (self.config.l2 is not None):
      w_reg    = 0.5*self.config.l2*tf.reduce_sum(tf.pow(W, 2))
      tf.add_to_collection("total_loss", w_reg)
      u_reg    = 0.5*self.config.l2*tf.reduce_sum(tf.pow(U, 2))
      tf.add_to_collection("total_loss", u_reg)
    ### END YOUR CODE
    return output 
  ...
  def add_loss_op(self, y):
    ...
    """
    ### YOUR CODE HERE
    # y: score prior to application of softmax
    xent = tf.nn.softmax_cross_entropy_with_logits(
      y, self.labels_placeholder, name="xent")
    xent_loss = tf.reduce_mean(xent, name="xent_loss")
    tf.add_to_collection("total_loss", xent_loss)
    loss_items = tf.get_collection("total_loss")
    loss = tf.reduce_sum(loss_items)
    ### END YOUR CODE
    return loss
  ...
  def __init__(self, config):
    ...
    self.config = config
    self.load_data(debug=False) # unset when running production
...
def print_confusion(confusion, num_to_tag):
    ...
    for i, tag in sorted(num_to_tag.items()):
      if (total_guessed_tags[i] == 0):
        prec = 1.0
      else:  
        prec = confusion[i, i] / float(total_guessed_tags[i])
      if (total_true_tags[i] == 0):
        recall = 1.0
      else:
      ...
      print 'Tag: {} - P {:2.4f} / R {:2.4f} /F {:2.4f}'.format(tag, prec, recall, f)
...
def create_dirs_and_get_filenames(config):
  dir_name = 'ner-hidden_size%d-dropout%f-lr%f-l2reg%f'% \
             (config.hidden_size, config.dropout, config.lr, config.l2)
  ...
  ner_weight_file = weight_dir + '/ner.weights'
  predict_file    = result_dir + '/q2_test.predicted'

  return (ner_weight_file, predict_file)
...
def test_NER(config):
  ...
  """
  ner_weight_file, predict_file = create_dirs_and_get_filenames(config)
    ...
    with tf.Session() as session:
      ...
      for epoch in xrange(config.max_epochs):
        ...
        if val_loss < best_val_loss:
          ...
          saver.save(session, ner_weight_file)
      ...
      saver.restore(session, ner_weight_file)
      print 'Test'
      print '=-=-='
      print 'Writing predictions to %s' % predict_file
      _, predictions = model.predict(session, model.X_test, model.y_test)
      save_predictions(predictions, predict_file)
      
  return best_val_loss

def find_best_NER():
  """ Try multiple NER combinations and find the best hyperparameters
  combination that provides best results on the validation test.
  """
  hidden_size_list = np.array([80, 100, 125])
  dropout_list = np.array([0.81, 0.9, 0.95])
  lr_list = np.logspace(-4,-2,4)
  l2_list = np.logspace(-4,-2,4)
  ...
  for config in config_list:
    result_list.append(test_NER(config))
  ...
  print "L2Regularizer  = %8.6f" % config.l2
  print "ValidationLoss = %f" % val
