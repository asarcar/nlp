...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    idxs = np.array([tokens[w] for w in sentence])
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    # features: N/d
    # weights:  d/|labels|
    # prob:     features*weights = (N/d)*(d/|labels|) = N/|labels|
    pred  = np.argmax(prob, axis=1)
    delta = prob
    delta[range(N), labels] -= 1
    # dim: (d/N)*(N/|labels|)=d/|labels|
    grad = features.T.dot(delta)/N + regularization*weights 
